...
async def upload_files(train_file: UploadFile = File(...), 
                       test_file: UploadFile = File(...), 
                       grammar: UploadFile = File(...)):
   try:
       train_dir = "../examples"
       ...
       grammar_path = os.path.join(train_dir, grammar.filename)
       ...
       if os.path.exists(grammar_path):
           raise HTTPException(status_code=400, detail=f"Grammar file '{grammar.filename}' already exists.")
       ...
       with open(grammar_path, "wb") as buffer:
           shutil.copyfileobj(grammar.file, buffer)
...
async def list_files():
    try:
        ...
        files = [f for f in os.listdir(file_dir) if f.endswith(('.train', '.test', '.bnf'))]
        return files
    except Exception as e:
